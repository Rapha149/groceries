...
/*               onIonBlur={(e) => { setSearchState(prevState => ({...prevState,isFocused: false}))}} */           >   
  ...
  for (let i = 0; i < pageState.itemRows.length; i++) {
    ...
    currentRows.push(
      <IonItem id={"item-row-"+item.itemID} className={"itemrow-outer "+(rowVisible ? "itemrow-display" : "itemrow-hidden")+(item.important ? " itemrow-important" : "")} key={"itemouter"+pageState.itemRows[i].itemID} >
